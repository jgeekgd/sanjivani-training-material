 ...
 * run on a Java Virtual Machine, or JVM for short. When Java programs run on
 * the JVM, objects are created on the heap, which is a portion of memory
 ...
 * negligence, at a certain point, sufficient memory may not be available to
 * create new objects, and the entire program will terminate abnormally, causing
 * OutOfMemoryErrors.
 * 
 * But in Java, the programmer need not care for all those objects which are no
 * longer in use. Garbage collector destroys these objects. The main objective
 * of Garbage Collector is to free heap memory by destroying unreachable
 * objects. The garbage collector is the best example of the Daemon thread as it
 ...
 * object can be reclaimed. The programmer does not need to mark objects to be
 ...
 * 
 * 1. Unreachable objects: An object is said to be unreachable if it doesn’t
 * contain any reference to it. Also, note that objects which are part of the
 * island of isolation are also unreachable.
 * 
 * Integer i = new Integer(4); // the new Integer object is reachable via the
 * reference in 'i'.
 * 
 ...
 * GC(garbage collection) if it is unreachable. After i = null, integer object 4
 * in the heap area is suitable for garbage collection in the above code.
 * 
 * 
 * Ways to make an object eligible for Garbage Collector Even though the
 * programmer is not responsible for destroying useless objects but it is highly
 * recommended to make an object unreachable(thus eligible for GC) if it is no
 * longer required.
 * 
 * 
 ...
 * to interface with the JVM in which the application is running. Hence by using
 * its gc() method, we can request JVM to run Garbage Collector.
 * 
 * There is no guarantee that any of the above two methods will run Garbage
 * Collector.
 * 
 ...
 * 
 * 
 * Based on our requirement, we can override finalize() method for performing
 * our cleanup activities like closing connection from the database.
 * 
 ...
 * 
 * 
 * The java.lang.System.runFinalization() method runs the finalization methods of any objects pending finalization.
 *
 */
