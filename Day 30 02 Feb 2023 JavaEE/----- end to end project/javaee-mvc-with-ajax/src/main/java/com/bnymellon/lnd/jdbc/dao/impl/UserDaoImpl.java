...
public class UserDaoImpl implements UserDao {
	...
	@Override
	public List<User> findAllUsersByLoginIdPattern(String loginIdPattern) {
		...
		try {
			connection = ConnectionFactory.getConnection();
			preparedStatment = connection.prepareStatement(QueryConstants.SELECT_USERS_BY_ID_PATTERN);
			preparedStatment.setString(1, "%" +loginIdPattern.toUpperCase()+"%");
			rs = preparedStatment.executeQuery();
			while (rs.next()) {
				currentUser = new User(rs.getString(1), rs.getString(2), rs.getString(3));
				foundList.add(currentUser);
